export class Type {
    ...
    constructor(pokemon_info){
        ...
        this.setDebilidadesyResistencias(this.types);
    }

    private setTypes(pokemon_info){
        pokemon_info['types'].forEach(elemento=>{
            this.types.push(elemento['type']['name']);
        })
    }

    private setDebilidadesyResistencias(types){
        let duplicado;
        let auxDebilidades = [];
        let debilidades = [];
        let resistencias = [];
        ...
        types.forEach(type => {
          debilidades = debilidades.concat(this.getDebilidades(type));
          resistencias = resistencias.concat(this.getresistencias(type));
        });
